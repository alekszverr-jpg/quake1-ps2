...
static qboolean keyboard_available;
static qboolean mouse_available;

void IN_Init (void)
{
	if (PS2_UsbInputAvailable())
	{
		keyboard_available = PS2KbdInit() != 0;
		mouse_available = PS2MouseInit() != 0;
	...
	{
		printf("USB keyboard and mouse disabled on host:; using gamepad input\n");
	}
...
#define L_RIGHT 255    
	...
	{ 
        ...
        case PAD_START: Key_Event(K_PAUSE, isPadDown);break; 
        ...
        case PAD_CIRCLE: Key_Event(K_SHIFT, isPadDown); break;
        ...
        case PAD_L1+PAD_R1: Key_Event(K_MOUSE3, isPadDown);Key_Event(K_MOUSE1, isPadDown); break;
        case PAD_CROSS+PAD_R1: Key_Event(K_ENTER, isPadDown);Key_Event(K_MOUSE1, isPadDown); break;
        case 0: isPadDown=false; Key_Event(K_LEFTARROW, isPadDown);
        Key_Event(K_DOWNARROW, isPadDown);Key_Event(K_RIGHTARROW, isPadDown);
        Key_Event(K_UPARROW, isPadDown);Key_Event(K_PAUSE, isPadDown); 
        Key_Event(K_TAB, isPadDown); Key_Event(K_SLA, isPadDown);
        Key_Event(K_ENTER, isPadDown); Key_Event(K_SHIFT, isPadDown);
...
float rjoy_h2=0, ljoy_v2=0;
	//0-122.5-127.5-132.5-255
       if(rjoy.rjoy_h<112){rjoy_h2=rjoy.rjoy_h;}else if(rjoy.rjoy_h>141){rjoy_h2=rjoy.rjoy_h;}else{rjoy_h2=122;}
       if(rjoy.ljoy_v<112){ljoy_v2=rjoy.ljoy_v;}else if(rjoy.ljoy_v>141){ljoy_v2=rjoy.ljoy_v;}else{ljoy_v2=122;}
 //| rjoy.rjoy_h>132.5
 //| rjoy.ljoy_v>132.5
#ifdef _MOUSE
